...
logic error_seen = 0; // error is a 1-cycle pulse
always_ff @(posedge clk)
...
task automatic send_frame(input logic [47:0] dst, input logic [47:0] src, input logic [15:0] etype, input logic [7:0] payload[], input logic [31:0] fcs);
    ...
    for (int i = 5; i >= 0; i--) send_byte(dst[i*8 +: 8]); //destination mac
 
    for (int i = 5; i >= 0; i--) send_byte(src[i*8 +: 8]); //src mac address
...
initial begin
    ...
    //test 1
    //dst=FF:FF:FF:FF:FF:FF, src=DE:AD:BE:EF:00:01
    //ethertype=0x0800, payload=DE AD BE EF CA FE 
    $display("\n=== Test 1: normal frame ===");
    rx_payload = '{};
 
    begin //for automatic vars need scope
    ...
    begin
        automatic logic [7:0] pl[] = '{8'h00, 8'h01, 8'h08, 8'h00,
                                       8'h06, 8'h04, 8'h00, 8'h01};
    ...
    check("error flag set",      error,      1'b1);
